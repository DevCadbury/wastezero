...
import { MatchedOpportunityPage, Opportunity, OpportunityPage } from '../models/models';
...
export class OpportunityService {
  ...
  /** Volunteer profile-based matched opportunities */
  listMatches(params?: { page?: number; limit?: number }): Observable<MatchedOpportunityPage> {
    let httpParams = new HttpParams();
    if (params?.page) httpParams = httpParams.set('page', params.page);
    if (params?.limit) httpParams = httpParams.set('limit', params.limit);
    return this.http.get<MatchedOpportunityPage>(`${this.apiUrl}/matches`, { params: httpParams });
  }
